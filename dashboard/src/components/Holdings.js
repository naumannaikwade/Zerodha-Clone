...
function Holdings() {
  ...
  const totalPnlPercent = totalInvestment > 0 ? ((totalCurrentValue - totalInvestment)/totalInvestment * 100).toFixed(2) : 0;
              ...
              <table className="holdings-table">
                      ...
                      <tr key={h._id}>
                        ...
                        <td>
                          <div className="holdings-pnl-container">
                            <span className={`holdings-pnl-amount ${pnl >= 0 ? 'positive' : 'negative'}`}>{formatPnl(pnl)}</span>
                            <span className={`holdings-pnl-percent ${pnlPercent >= 0 ? 'positive' : 'negative'}`}>{formatPercent(pnlPercent)}</span>
                          </div>
                        </td>
                      </tr>
                    );
                ...
                <tfoot>
                  <tr className="holdings-summary-row">
                    ...
                    <td>
                      <div className="holdings-pnl-container">
                        <span className={`holdings-pnl-amount ${totalPnl >= 0 ? 'positive' : 'negative'}`}>{formatPnl(totalPnl)}</span>
                        <span className={`holdings-pnl-percent ${totalPnlPercent >= 0 ? 'positive' : 'negative'}`}>{formatPercent(totalPnlPercent)}</span>
                      </div>
                    </td>
                  </tr>
                </tfoot>
