...
// Sell stock - updates Orders, Holdings, Positions, and Funds
router.post("/sell", async (req, res) => {
  try {
    const { symbol, price, quantity } = req.body;

    console.log("💸 Sell request:", { symbol, price, quantity, userId: req.user.id });

    // 1️⃣ Find holding
    const holding = await Holding.findOne({ userId: req.user.id, symbol });
    if (!holding) {
      return res.status(400).json({ success: false, message: "You don’t own any shares of this stock." });
    }
    ...
    // 2️⃣ Calculate profit/loss and update holding
    ...
    // 3️⃣ Update user funds (add proceeds)
    let funds = await Funds.findOne({ userId: req.user.id });
    if (!funds) return res.status(404).json({ success: false, message: "Funds not found" });
